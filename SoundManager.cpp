...
SoundManager::SoundManager()
{
	...
	//load the sound in to buffers from the file
	...
	//associate the sound with the buffers
	...
	//when the player is 50 pixels or any closer, the sound is full volume
	float minDistance = 150;

	//the sound reduces steadily as the player moves further away
	float attentuation = 15;

	//set attentuation levels for sounds
...
}	//end of sound manager constructor
...
void SoundManager::playFire(Vector2f emitterlocation, Vector2f listenerlocation)
{
	...
	//go back to 1 when the thrid sound has started
	if (m_NextSound > 3)
	{
		m_NextSound = 1;
	}
